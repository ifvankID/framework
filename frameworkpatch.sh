...
# --- Step 1: Copy File from sdcard ---
if [[ ! -f "$source_file" ]]; then
    echo "ERROR: $framework_name not found in $sdcard_path!"
    exit 1
fi
cp "$source_file" "$work_dir/$framework_name" > /dev/null 2>&1
if [[ $? -ne 0 ]]; then
    echo "ERROR: Failed to copy. Ensure Termux has storage permission."
    exit 1
fi
...
keystorespiclassfile=$(find ifvank/ -name 'AndroidKeyStoreSpi.smali' -printf '%P\n')
instrumentationsmali=$(find ifvank/ -name "Instrumentation.smali"  -printf '%P\n')
pm_smali=$(find ifvank/ -name 'ApplicationPackageManager.smali' -printf '%P\n')
if [ -z "$pm_smali" ]; then
    echo "ERROR: Could not find ApplicationPackageManager.smali!"
    exit 1
fi
engineGetCertMethod=$(expressions_fix "$(grep 'engineGetCertificateChain(' "ifvank/$keystorespiclassfile")")
newAppMethod1=$(expressions_fix "$(grep 'newApplication(Ljava/lang/ClassLoader;' "ifvank/$instrumentationsmali")")
newAppMethod2=$(expressions_fix "$(grep 'newApplication(Ljava/lang/Class;' "ifvank/$instrumentationsmali")")
sed -n "/^${engineGetCertMethod}/,/^\.end method/p" "ifvank/$keystorespiclassfile" > tmp_keystore
sed -i "/^${engineGetCertMethod}/,/^\.end method/d" "ifvank/$keystorespiclassfile"
sed -n "/^${newAppMethod1}/,/^\.end method/p" "ifvank/$instrumentationsmali" > inst1
sed -i "/^${newAppMethod1}/,/^\.end method/d" "ifvank/$instrumentationsmali"
sed -n "/^${newAppMethod2}/,/^\.end method/p" "ifvank/$instrumentationsmali" > inst2
sed -i "/^${newAppMethod2}/,/^\.end method/d" "ifvank/$instrumentationsmali"
...
cat inst1 >> "ifvank/$instrumentationsmali"
cat inst2 >> "ifvank/$instrumentationsmali"
cat tmp_keystore >> "ifvank/$keystorespiclassfile"
...
sed -i "/# static fields/a ${FEATURES_FIELDS}" "ifvank/$pm_smali"
CODENAME_FIELD=".field private static final whitelist pTensorCodenames:[Ljava/lang/String;"
sed -i "/\.field private static final greylist-max-o sDefaultFlags:I = 0x400/i ${CODENAME_FIELD}" "ifvank/$pm_smali"
...
perl -0777 -i -pe 's/$ENV{FIND_REGEX}/$ENV{REPLACE_TEXT}/' "ifvank/$pm_smali"
...
if [[ -f "PIF/classes.dex" ]]; then
    ...
    if [ ! -d "classes5" ]; then
        echo "ERROR: Failed to decompile PIF/classes.dex!"
        exit 1
    fi
    rm -rf "ifvank/smali_classes5"
    mv "classes5" "ifvank/smali/classes5"
    if [ ! -d "ifvank/smali/classes5" ]; then
        echo "ERROR: Failed to move smali files from PIF!"
        exit 1
    fi
fi

echo "[*] Repacking framework.jar..."
apkeditor b -i ifvank -o framework-patched.apk
if [[ ! -f "framework-patched.apk" ]]; then
    echo "ERROR: Repacking failed, file not found."
    exit 1
fi
mv framework-patched.apk framework-final.jar
if [[ ! -f "framework-final.jar" ]]; then
    echo "ERROR: Final file framework-final.jar was not created."
    exit 1
fi
mv "framework-final.jar" "$source_file"
if [[ $? -ne 0 ]]; then
    echo "ERROR: Failed to move file to sdcard. Check permissions."
    exit 1
fi
