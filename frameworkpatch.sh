...
baksmali() {
    jarfile=$dirnow/tool/baksmali.jar
    if [[ ! -f $jarfile ]]; then
        echo "ERROR: baksmali.jar not found in tool/ folder! Please download it."
        exit 1
    fi
    java -jar "$jarfile" "$@"
}
...
blSpoofPatch() {
	blSpoofPatch="    invoke-static {$1}, Lcom/android/internal/util/ifvank/util/framework/KeyboxUtils;->engineGetCertificateChain([Ljava/security/cert/Certificate;)[Ljava/security/cert/Certificate;	
...
pm_smali=$(find frmwrk/ -name 'ApplicationPackageManager.smali' -printf '%P\n')
if [ -z "$pm_smali" ]; then
    echo "ERROR: Could not find ApplicationPackageManager.smali in frmwrk/"
    exit 1
fi
...
sed -i "/# static fields/a ${FEATURES_FIELDS}" "frmwrk/$pm_smali"
CODENAME_FIELD=".field private static final whitelist pTensorCodenames:[Ljava/lang/String;"
sed -i "/\.field private static final greylist-max-o sDefaultFlags:I = 0x400/i ${CODENAME_FIELD}" "frmwrk/$pm_smali"
...
perl -0777 -i -pe 's/$ENV{FIND_REGEX}/$ENV{REPLACE_TEXT}/' "frmwrk/$pm_smali"
...
if [[ -f "PIF/classes.dex" ]]; then
    ...
    if [ ! -d "classes5" ]; then
        echo "ERROR: Decompiling PIF/classes.dex failed. Aborting."
        exit 1
    fi
    rm -rf "frmwrk/smali_classes5"
    mv "classes5" "frmwrk/smali/classes5"
    if [ ! -d "frmwrk/smali/classes5" ]; then
        echo "ERROR: Failed to move smali files. Aborting."
        exit 1
    fi
fi
...
unzip -o frmwrk_out.apk 'classes*.dex' -d frmwrk > /dev/null 2>&1
rm -rf frmwrk/.cache
cd frmwrk
zip -qr0 -t 07302003 $dirnow/frmwrk.jar classes*
cd $dirnow
mv frmwrk.jar framework.jar
